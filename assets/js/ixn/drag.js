// svg.drag.js 0.1.0 - Copyright (c) 2014 Wout Fierens - Licensed under the MIT license
// extended by Florian Loch; modified by Daniel Suo
;
(function() {

  SVG.extend(SVG.Element, {
    // Enable drag on window or element
    // Constraint might be a object (as described in readme.md) or a function in the form "function (x, y)" that gets called before every move.
    // The function can return a boolean or a object of the form {x, y}, to which the element will be moved. "False" skips moving, true moves to raw x, y.
    enableDrag: function(dragWindow, constraint) {
      var element = this;

      element.dragWindow = dragWindow || false;

      var parent = this.parent._parent(SVG.Nested) || this._parent(SVG.Doc);

      /* disable drag if already present */
      if (typeof this.disableDrag === 'function')
        this.disableDrag()

      /* ensure constraint object */
      constraint = constraint || {}

      /* start dragging */
      element.start = function(event) {
        event = event || window.event

        /* invoke any callbacks */
        if (element.beforedrag)
          element.beforedrag(event)

        /* get element bounding box */
        var box = element.bbox()

        if (element.dragWindow) {
          box.x = -parent.viewbox().x;
          box.y = -parent.viewbox().y;
          box.width = parent.viewbox().width;
          box.height = parent.viewbox().height;
        } else {

          if (element instanceof SVG.G) {
            box.x = element.x()
            box.y = element.y()

          } else if (element instanceof SVG.Nested) {
            box = {
              x: element.x(),
              y: element.y(),
              width: element.width(),
              height: element.height()
            }
          }
        }

        /* store event */
        element.startEvent = event

        /* store start position */
        element.startPosition = {
          x: box.x,
          y: box.y,
          width: box.width,
          height: box.height,
          zoom: parent.viewbox().zoom,
          rotation: element.transform('rotation') * Math.PI / 180
        }

        /* add while and end events to window */
        SVG.on(window, 'mousemove', element.drag)
        SVG.on(window, 'mouseup', element.end)

        /* invoke any callbacks */
        if (element.dragstart)
          element.dragstart({
            x: 0,
            y: 0,
            zoom: element.startPosition.zoom
          }, event)

        /* prevent selection dragging */
        event.preventDefault ? event.preventDefault() : event.returnValue = false
      }

      /* while dragging */
      element.drag = function(event) {
        event = event || window.event

        if (element.startEvent) {
          /* calculate move position */
          var x, y, rotation = element.startPosition.rotation,
            width = element.startPosition.width,
            height = element.startPosition.height,
            delta = {
              x: event.clientX - element.startEvent.clientX,
              y: event.clientY - element.startEvent.clientY,
              zoom: element.startPosition.zoom
            }

          /* caculate new position [with rotation correction] */
          x = element.startPosition.x + (delta.x * Math.cos(rotation) + delta.y * Math.sin(rotation)) / element.startPosition.zoom
          y = element.startPosition.y + (delta.y * Math.cos(rotation) + delta.x * Math.sin(-rotation)) / element.startPosition.zoom

          /* move the element to its new position, if possible by constraint */
          // if (typeof constraint === 'function') {
          //   var coord = constraint(x, y)

          //   if (typeof coord === 'object') {
          //     if (typeof coord.x != 'boolean' || coord.x)
          //       element.x(typeof coord.x === 'number' ? coord.x : x)
          //     if (typeof coord.y != 'boolean' || coord.y)
          //       element.y(typeof coord.y === 'number' ? coord.y : y)

          //   } else if (typeof coord === 'boolean' && coord) {
          //     element.move(x, y)
          //   }

          // } else if (typeof constraint === 'object') {
          //   /* keep element within constrained box */
          //   if (constraint.minX != null && x < constraint.minX)
          //     x = constraint.minX
          //   else if (constraint.maxX != null && x > constraint.maxX - width)
          //     x = constraint.maxX - width

          //   if (constraint.minY != null && y < constraint.minY)
          //     y = constraint.minY
          //   else if (constraint.maxY != null && y > constraint.maxY - height)
          //     y = constraint.maxY - height

          //   element.move(x, y)
          // }

          if (element.dragWindow) {
            var viewbox = parent.viewbox();
            parent.viewbox(-x, -y, viewbox.width, viewbox.height);
          } else {
            element.move(x, y);
          }

          /* invoke any callbacks */
          if (element.dragmove)
            element.dragmove(delta, event)
        }
      }

      /* when dragging ends */
      element.end = function(event) {
        event = event || window.event

        /* calculate move position */
        var delta = {
          x: event.clientX - element.startEvent.clientX,
          y: event.clientY - element.startEvent.clientY,
          zoom: element.startPosition.zoom
        }

        /* reset store */
        element.startEvent = null
        element.startPosition = null

        /* remove while and end events to window */
        SVG.off(window, 'mousemove', element.drag)
        SVG.off(window, 'mouseup', element.end)

        /* invoke any callbacks */
        if (element.dragend)
          element.dragend(delta, event)
      }

      /* bind mousedown event */
      if (element.dragWindow) {
        SVG.on(window, 'mousedown', element.start);
      } else {
        // element.mousedown(element.start);
        element.node.addEventListener('mousedown', element.start, false);
      }

      /* disable draggable */
      element.disableDrag = function() {
        if (element.dragWindow) {
          SVG.off(window, 'mousedown', element.start);
        } else {
          element.node.removeEventListener('mousedown', element.start, false);
        }

        SVG.off(window, 'mousemove', element.drag)
        SVG.off(window, 'mouseup', element.end)

        return element
      }

      return this
    }

  })

}).call(this);