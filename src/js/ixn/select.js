...
(function() {
    ...
    SVG.extend(SVG.Element, {
        ...
        select: function(value, options) {
            ...
            var defaults, observer, element = this,
                bbox = this.bbox(),
            ...
            if (defaults.deepSelect && ['line', 'polyline', 'polygon'].indexOf(element.type) !== -1) {
                            ...
                            .mousedown(
                                (function(k) {
                                    return function(ev) {
                                        ...
                                        element.node.dispatchEvent(new CustomEvent('point', {
                                            detail: {
                                                x: ev.pageX,
                                                y: ev.pageY,
                                                i: k
                ...
                if (MutationObserver) {
                    ...
                    this.deepSelect.attributeObserver.observe(this.node, {
                        attributes: true
                    });
            ...
            function getMoseDownFunc(eventName) {
                return function(ev) {
                    ...
                    element.node.dispatchEvent(new CustomEvent(eventName, {
                        detail: {
                            x: ev.pageX,
                            y: ev.pageY
                        }
            ...
            if (defaults.rotationPoint) {
                this.boundingGroup.circles.push(this.boundingGroup.group.circle(defaults.radius).center(bbox.width / 2, 20).attr('class', defaults.classPoints + '_rot')
                    .mousedown(function(ev) {
                        ev.preventDefault && ev.preventDefault();
                        element.node.dispatchEvent(new CustomEvent('rot', {
                            detail: {
                                x: ev.pageX,
                                y: ev.pageY
                            }
            ...
            if (MutationObserver) {
                ...
                this.boundingGroup.attributeObserver.observe(this.node, {
                    attributes: true
                });
